...
class VehicleModelControllerTest extends TestCase
{
    ...
    public function testFetchByBrandIdWithExistingBrand()
    {
        $brand = Brand::factory()->create();
        VehicleModel::factory()->count(3)->create(['brand_id' => $brand->id]);

        $response = $this->get('/api/v1/public/vehicle-models/' . $brand->id);

        $response->assertStatus(200)
            ->assertJsonCount(3);
    }
    public function testFetchByBrandIdWithNonExistingBrand()
    {
        $response = $this->get('/api/v1/public/vehicle-models/999');

        $response->assertStatus(404)
            ->assertJson(['error' => 'not found brand']);
    }
...
//
//        // Act: Make request
//        $response = $this->getJson('/api/v1/public/vehicle-model');
//
